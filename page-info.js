function pgHowToUse(){
  ...
  hd.appendChild(el("div",{css:{fontSize:".92rem",color:"var(--muted)",lineHeight:"1.7",maxWidth:"500px",margin:"0 auto"},txt:"Everything you need to know to get the most out of your study sessions. Follow these steps and start learning smarter!"}));
  wrap.appendChild(hd);
  ...
  steps.forEach(function(step){
    var card = el("div",{cls:"htu-step-card",css:{background:"var(--card)",border:"1px solid var(--border)",borderRadius:"16px",padding:"24px 28px",marginBottom:"16px",position:"relative",overflow:"hidden",transition:"box-shadow .2s,border-color .2s"}});
    // Accent left bar
    ...
    // Top row
    var topRow = el("div",{cls:"htu-top-row",css:{display:"flex",alignItems:"flex-start",gap:"16px",marginBottom:"12px"}});
    // Number badge
    var numBadge = el("div",{css:{minWidth:"40px",height:"40px",borderRadius:"10px",background:step.color+"22",border:"1.5px solid "+step.color+"44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:".68rem",fontWeight:"800",color:step.color,letterSpacing:".06em",fontFamily:"var(--font-display)"}},step.num);
    ...
    titleBlock.appendChild(el("div",{css:{display:"flex",alignItems:"center",gap:"8px",marginBottom:"4px"}},[
      ...
      el("span",{css:{fontSize:"1rem",fontWeight:"700",letterSpacing:"-.01em"}},step.title)
    ]));
    ...
    // Tips
    var tipsWrap = el("div",{cls:"htu-tips-wrap",css:{marginLeft:"56px",display:"flex",flexDirection:"column",gap:"5px"}});
    step.tips.forEach(function(tip){
      var t = el("div",{css:{display:"flex",alignItems:"baseline",gap:"8px",fontSize:".8rem",color:"var(--muted)"}});
      t.appendChild(el("span",{css:{color:step.color,fontWeight:"700",flexShrink:"0"}}, "→"));
      t.appendChild(el("span",{},tip));
      tipsWrap.appendChild(t);
    });
  ...
  var footer = el("div",{css:{background:"linear-gradient(135deg,rgba(79,142,247,0.08),rgba(126,179,255,0.04))",border:"1px solid rgba(79,142,247,0.2)",borderRadius:"16px",padding:"24px 28px",marginBottom:"24px",textAlign:"center"}});
  ...
  footer.appendChild(el("div",{css:{fontSize:".88rem",color:"var(--muted)",marginBottom:"18px",lineHeight:"1.6"}},
    "The best way to prepare is consistency — even 15 minutes a day makes a big difference. Start with a subject you enjoy, build the habit, and let StudyLab track your growth."));
  footer.appendChild(el("button",{cls:"btn btnp",css:{padding:"11px 28px",fontSize:".9rem"},onclick:function(){go("home");}}, "🏠 Go to Home →"));
...
function pgAbout(){
  ...
  // Stats row
var tot = SUBJ.reduce(function(s, k) { return s + (QD[k] || []).length; }, 0); // Calculates total questions
var subjCount = SUBJ.length; // Calculates total subjects
...
[[tot.toLocaleString() + "+", "MCQs"], [subjCount.toString(), "Subjects"], ["100%", "Free"], ["0", "Ads"]].forEach(function(s){
  ...
  cinfo.appendChild(el("div",{css:{fontSize:".88rem",color:"var(--muted)",lineHeight:"1.75",fontWeight:"300"}},
    "I'm a student preparing for government exams — UPSC, SSC, and state PCS. I built StudyLab for myself because I couldn't find a clean, distraction-free tool that actually fit my study flow. No ads, no paywalls, just focused prep."));
  cinfo.appendChild(el("div",{css:{fontSize:".85rem",color:"var(--muted)",lineHeight:"1.7",fontWeight:"300",marginTop:"8px"}},
    "If this app helps even one fellow aspirant crack their exam, that's more than enough for me. We're all in this together — best of luck to every one of you. 💪"));
  ...
  [
    ...
    ["🎯","Quiz Mode","MCQ sessions with instant feedback & history"],
    ["🔔","Govt Updates","Live vacancies, admit cards & results via RSS"],
    ["📊","Progress Tracker","Accuracy, streaks & subject-wise breakdown"],
    ["🎯","Daily Challenge","Fresh mixed quiz every day to build habit"],
    ["☁️","Cloud Sync","Sign in with Google to save progress everywhere"],
  ...
  var faqs = [
    ...
    ["Do I need an account?", "No account needed to study. Sign in with Google only if you want to sync your progress across devices."],
    ["Which exams is this useful for?", "Primarily for UPSC, SSC, RRB, State PCS and other competitive govt exams that test General Studies."],
    ["How is the question data sourced?", "Questions are curated from standard GS preparation books and verified for accuracy."],
    ["Can I use it offline?", "Once loaded, most features work without internet. Gov Updates requires a connection to fetch live data."],
    ["How do I report a mistake in a question?", "For now, you can reach out via the contact links below. Feedback is always welcome!"]
  ];
  var faqOpen = {};
  ...
  contactBox.appendChild(el("div",{css:{fontSize:".88rem",color:"var(--muted)",marginBottom:"16px",lineHeight:"1.65",fontWeight:"300"}},
    "Have a suggestion, found a bug, or just want to say hi? Feel free to reach out. Every bit of feedback helps make StudyLab better for everyone."));
