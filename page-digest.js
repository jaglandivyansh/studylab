...
function pgDigest() {
  ...
  // ── ROUTING INTERCEPT ──
  // If the user clicked a category, 'sub' will hold the id (e.g. "national")
  if (sub) {
    w.appendChild(renderDigestSubPage(sub, CATS));
    return w;
  }
  ...
  CATS.forEach(function (cat, idx) {
    ...
    var row = el("div", {
      ...
      // CHANGED: Navigates to the sub-page using your standard router
      onclick: function () { go("digest", cat.id); } 
    });
...
// ── DEDICATED SUB-PAGE FOR NEWS ──
function renderDigestSubPage(catId, CATS) {
  var cat = CATS.find(function(c) { return c.id === catId; });
  if (!cat) return el("div", { css: { padding: "40px", textAlign: "center" } }, "Category not found.");
  ...
  var backBtn = el("button", {
    ...
    onclick: function() { go("digest", null); } 
  });
  
  // Clean text and a simple arrow that matches the home page typography
  ...
  var panel = el("div", { css: {
    ...
    borderRadius: "16px", padding: "22px", boxShadow: "var(--shadow-card)"
  }});
  ...
  // Gracefully handle CA_FEEDS and CA_FALLBACK arrays
  ...
  // Instantly render cached articles if we have them
